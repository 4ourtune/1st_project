...
#define CALCULATE_DISTANCE(fall_time, rise_time) (0.343F * ((fall_time) - (rise_time)) / 2.0F) // mm/us
...
void Ultrasonic_EchoHandler (UltrasonicSide side, int input_pin_state)
{
    ...
    if (input_pin_state)
    {
        rise_time_us[side] = getTimeUs();
    }
    else
    {
        uint64_t fall_time = getTimeUs();
        float raw_dist = CALCULATE_DISTANCE(fall_time, rise_time_us[side]);
        float filt_dist = Filter_Update(&filters[side], raw_dist);

        ult_data[side].distance_mm = (unsigned int) filt_dist;
        ult_data[side].timestamp_us = rise_time_us[side];
